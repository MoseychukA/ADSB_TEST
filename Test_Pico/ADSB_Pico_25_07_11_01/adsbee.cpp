...
bool ADSBee::Update() 
{
    ...
    while (/*decoder.decoded_1090_packet_out_queue.Pop(decoded_packet)*/ raw_1090_packet_queue.Pop(raw_packet)) 
    {
      ...
      //  comms_manager.console_printf("ADSBee::Update", "\tdf=%d icao_address=0x%06x", decoded_packet.GetDownlinkFormat(), decoded_packet.GetICAOAddress());
       // CONSOLE_INFO("ADSBee::Update", "\tdf=%d icao_address=0x%06x", decoded_packet.GetDownlinkFormat(), decoded_packet.GetICAOAddress());
        ...
        if (aircraft_dictionary.IngestDecoded1090Packet(decoded_packet)) 
        {
            ...
            comms_manager.console_printf("ADSBee::Update", "\taircraft_dictionary: %d aircraft", aircraft_dictionary.GetNumAircraft());
        }
        comms_manager.transponder_packet_reporting_queue.Push(decoded_packet);
    }
