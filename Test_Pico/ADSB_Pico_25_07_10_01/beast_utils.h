...
// Структура кадра Beast
// <предваряется символом escape 0x1a>
// Тип кадра 1 байт (не требует escape).
// RSSI 1-2 байта (может потребоваться escape-байт).
// Метка времени MLAT 6-12 байт (может потребоваться 6x escape-байтов).
// Данные Mode-S (2 байта + escape-символы для Mode A/C, 7 байтов + escape-символы для сквиттера, 14 байтов + escape-символы для расширенного
// сквиттера)
