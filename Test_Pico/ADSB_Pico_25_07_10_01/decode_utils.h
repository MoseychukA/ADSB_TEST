...
const uint16_t kCPRNz = 15;                           // количество широтных зон между экватором и полюсом
const float kCPRdLatEven = 360.0f / (4 * kCPRNz);     // размер широтной зоны дл€ четного сообщени€
const float kCPRdLatOdd = 360.0f / (4 * kCPRNz - 1);  // размер широтной зоны дл€ нечетного сообщени€
...
/**
* ѕреобразует числовое значение в символ ASCII дл€ использовани€ при декодировании позывных.
* @param[in] value 6-битное значение дл€ поиска.
* @retval —имвол, соответствующий входному значению. ≈сли входное значение недопустимо или не используетс€, возвращает '#'.
*/
