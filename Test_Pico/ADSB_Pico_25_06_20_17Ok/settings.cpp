...
bool SettingsManager::Load() 
{
    ...
    {
        ResetToDefaults();
        //if (bsp.has_eeprom && !eepromPico.Save(settings)) 
        //{
...
bool SettingsManager::Save()
{
    ...
    if (bsp.has_eeprom) 
    {
        return eepromPico.Save(settings);
    }
    else 
    {
        FlashUtils::FlashSafe();
...
bool SettingsManager::SetDeviceInfo(const DeviceInfo &device_info) 
{
    if (bsp.has_eeprom) 
    {
        ...
        if (eepromPico.RequiresInit()) return false;
        return eepromPico.Save(device_info, kEEPROMDeviceInfoOffset);
    }
    else 
    {
...
bool SettingsManager::GetDeviceInfo(DeviceInfo &device_info) 
{
    if (bsp.has_eeprom) 
    {
        // Device Info is stored on external EEPROM.
        if (eepromPico.RequiresInit()) return false;
        return eepromPico.Load(device_info, kEEPROMDeviceInfoOffset);
    }
    else 
    {
        // Device Info is stored in flash.
