...
bool PacketDecoder::UpdateLogLoop() 
{
    ...
    for (uint16_t i = 0; i < num_messages; i++) 
    {
        ...
        switch (message.log_level) 
        {
            case SettingsManager::LogLevel::kInfo:
                comms_manager.console_printf("PacketDecoder::DecoderLoop", "%s", message.message);
                break;
    ...
    {
        comms_manager.console_printf("PacketDecoder::DecoderLoop", "Corrected single bit error at bit index %d.", bit_flip_index);
    }
    return true;
}
