...
{
   public:
    static constexpr uint16_t kPacketQueueLen = 100;       // ƒлина очереди пакетов
    static constexpr uint16_t kDebugMessageQueueLen = 100;//!! 20;  // ƒлина очереди сообщений отладки
    ...
    /**
    * —труктура, используема€ дл€ отправки сообщений отладки в основное €дро, поскольку PacketDecoder предназначен дл€ работы на нескольких €драх.
     */
    struct DebugMessage
    {
    ...
    /**
    * ¬ывести все отладочные сообщени€, сгенерированные UpdateDecoderLoop(). —ледует запускать из основного цикла, чтобы избежать
    * смешивани€ отладочных сообщений от декодера с отладочными сообщени€ми из основного цикла.
    */
    bool UpdateLogLoop();

    /**
    * —читывание RawTransponderPackets, попытка исправлени€ контрольной суммы и передача их в выходную очередь как
