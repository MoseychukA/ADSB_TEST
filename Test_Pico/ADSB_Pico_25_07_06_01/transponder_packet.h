...
class Decoded1090Packet {
   public:
    ...
    // Конструкторы
    /**
    * Конструктор Decoded1090Packet.
    * @param[in] rx_buffer Буфер для чтения. Должен быть упакован так, чтобы все 32 бита каждого слова были заполнены, а каждое
    * слово слева (MSb) выровнено так, чтобы общее количество бит было 112. Слова должны быть с обратным порядком байтов, причем MSb
    ...
    * @param[in] sigs_dbm RSSI полученного пакета в дБм. По умолчанию INT32_MIN, если не установлено.
    * @param[in] mlat_48mhz_64bit_counts Количество 12-мегагерцовых часов, используемых для 6-байтовой временной метки мультилатерации.
    */
    ...
    /**
    * Конструктор Decoded1090Packet из строки.
    * @param[in] rx_string Строка полубайтов в виде шестнадцатеричных символов. Big-endian, MSB (самый старый байт) первым.
    * @param[in] sigs_dbm RSSI полученного пакета в дБм. По умолчанию INT32_MIN, если не установлено.
    * @param[in] mlat_12mhz_counts Количество часов 12 МГц, используемых для 6-байтовой временной метки мультилатерации.
    */
    ...
    */
    ...
    /**
    * Устанавливает действительность пакета на true. Используется для маркировки 56-битных (Squitter) пакетов как действительных 
    * после их внешней проверки по словарю самолета, поскольку они не имеют 0 CRC и, таким образом, устанавливаются как недействительные
    * при создании.
    */
    ...
    /**
    * Сбрасывает внутренний буфер пакетов в место назначения и возвращает количество записанных байтов.
    ...
    */
    ...
    /**
    * Вычисляет 24-битную контрольную сумму CRC пакета ADS-B и возвращает значение контрольной суммы. Возвращаемое
    * значение должно соответствовать последним 24 битам в 112-битном пакете ADS-B, если пакет действителен.
