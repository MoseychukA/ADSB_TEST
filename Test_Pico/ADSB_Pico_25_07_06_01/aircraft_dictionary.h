...
class Aircraft1090 {
   public:
    ...
    static constexpr uint16_t kCallSignMinNumChars = 3;           // Позывные должны быть именно такой длины, чтобы быть действительными.
    // Эти переменные определяют границы фильтра для времени между пакетами CPR. Если время между пакетами больше, чем
    // предел дельты времени, старый пакет CPR отбрасывается, а пара пакетов CPR не используется для декодирования положения.
    static constexpr uint32_t kDefaultCPRIntervalMs = 10e3;       // Интервал CPR при запуске с нуля или устаревшего трека.
    static constexpr uint32_t kRefCPRIntervalMs = 19e3;           // Интервал отсчета для отклонения пар пакетов CPR.
    static constexpr uint32_t kMaxCPRIntervalMs = 30e3;           // Никогда не принимайте пары пакетов CPR с интервалом более 30 секунд.
    static constexpr uint32_t kMaxTrackUpdateIntervalMs = 20e3;   // Треки старше этого значения считаются устаревшими.
