...
class ADSBee 
{
   public:
    ...
    static constexpr int32_t kNoiseFloorExpoFilterPercent    = 50;  // [%] Вес, используемый для экспоненциального фильтра нижних частот шумового порога АЦП. 0 = без фильтра, 100 = удержание значения.
    static constexpr uint32_t kNoiseFloorADCSampleIntervalMs = 1;   // [мс] Интервал между выборками АЦП для приблизительного значения минимального уровня шума.
