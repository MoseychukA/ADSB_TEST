...
void Decoded1090Packet::ConstructTransponderPacket()
{
    if (raw_.buffer_len_bits != Raw1090Packet::kExtendedSquitterPacketLenBits && raw_.buffer_len_bits != Raw1090Packet::kSquitterPacketLenBits) // Не равен 112 или 56
    {
        ...
        return;  // leave is_valid_ as false // оставьте is_valid_ как false
    }

    // raw_.buffer[64] = {0x8D155C1D001B60000000006D69BB};

    //raw_.buffer[64] = 0x8D;// 1 5 5 C 1 D 0 0 1 B 6 0 0 0 0 0 0 0 0 0 6 D 6 9 B B
    //raw_.buffer[64] = 0x15;
    //raw_.buffer[64] = 0x5C;
    //raw_.buffer[64] = 0x1D;
    //    raw_.buffer[64] = 0x
        ...
        default:  // Все остальные DF. Примечание: DF=17-19 для ADS-B.
        {
           ...
           //!! comms_manager.console_printf("icao_address_ %x\r\n", icao_address_);
            ...
            else 
            {
               ...
               //!! comms_manager.console_printf("!!!Invalid checksum, expected %06lx but calculated %06lx.\r\n", parity_value, calculated_checksum);
            }
        }
    }
}
