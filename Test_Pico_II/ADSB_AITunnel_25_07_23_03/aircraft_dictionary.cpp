...
// Эта скорость определяет максимальное расстояние, на которое может перепрыгнуть самолет между обновлениями положения
//  без необходимости в повторном образце для подтверждения его нового положения. Это позволяет перехватывать отчеты о
//  положении, которые возвращаются как действительные из декодера CPR, но относятся к явно неверному положению.
...
{
    // TODO: Здесь может быть условие, при котором мы временно теряем пакеты, когда счетчик MLAT циклически обнуляется и
    ...
    {
        // Отклонить пары пакетов СЛР, которые слишком сильно разнесены по времени.
        WriteBitFlag(BitFlag::kBitFlagPositionValid, false);  // Отклонить пару пакетов SLR, которые слишком сильно изменились со временем.
        //CONSOLE_WARNING("Aircraft1090::DecodePosition",
...
{
    // ВНИМАНИЕ: Здесь задействованы две отдельные временные базы! Временная база для временных меток пакетов CPR — это MLAT
    // временная база, в то время как информация словаря самолетов более высокого уровня находится в системном времени.

    if (!CanDecodePosition()) 
    {
        // Невозможно попытаться принудительно декодировать и проверить, является ли это допустимым,
    ...
    if (!result_valid) 
    {
        ...
        return false;  // Самолет пересек две широтные зоны, не удается декодировать эту пару пакетов.
    }
...
bool AircraftDictionary::IngestADSBPacket(ADSBPacket packet) 
{
    ...
    if (aircraft_ptr == nullptr) 
    {
        comms_manager.console_printf("AircraftDictionary::IngestADSBPacket Unable to find or create new aircraft with ICAO address 0x%lx in dictionary.\r\n", icao_address);
        return false;  // unable to find or create new aircraft in dictionary
    }
    aircraft_ptr->last_message_timestamp_ms = millis();
...
bool Aircraft1090::SetCPRLatLon(uint32_t n_lat_cpr, uint32_t n_lon_cpr, bool odd, uint32_t received_timestamp_ms) 
{
    ...
    {
       // CONSOLE_ERROR("Aircraft1090::SetCPRLatLon", "Received CPR packet with out of bounds lat/lon values (%lu, %lu).", n_lat_cpr, n_lon_cpr);
 ...
 */
 /**
 * Возвращает категорию вихревого следа воздушного судна, отправившего заданный пакет ADS-B. Обратите внимание, что некоторые категории имеют
 * сопоставление «многие к одному»!
 * @param[in] пакет Пакет ADS-B для извлечения значения категории. Должен быть
 * @retval Категория, соответствующая комбинации возможностей и кода типа из пакета ADS-B, или
 * kCategoryInvalid, если нет соответствующего значения вихревого следа.
 */
...
bool AircraftDictionary::ApplyAirbornePositionMessage(Aircraft1090 &aircraft, ADSBPacket packet) {
    ...
    // ME[7] - Дополнение NIC B (ранее флаг одной антенны)
