...
bool ADSBee::Update()
{
    ...
    while (/*decoder.decoded_1090_packet_out_queue.Pop(decoded_packet) */raw_1090_packet_queue.Pop(raw_packet)) //raw_1090_packet_queue
    {
        ...
        if (aircraft_dictionary.IngestDecoded1090Packet(decoded_packet))
        {
            ...
            comms_manager.console_printf("ADSBee::Update \taircraft_dictionary: %d aircraft\r\n", aircraft_dictionary.GetNumAircraft());
        }
        comms_manager.transponder_packet_reporting_queue.Push(decoded_packet);
    }
    ...
    for (uint16_t sm_index = 0; sm_index < /*bsp.*/r1090_num_demod_state_machines; sm_index++) 
    {
       ...
       // aircraft_dictionary.Record1090Demod();
        ...
        for (uint16_t i = 0; i < packet_num_words; i++) 
        {
            ...
            {
                //!!  rx_packet_[sm_index].buffer[i] >>= 1;  // Обрезаем лишний принятый бит из последнего слова в пакете.
