...
class ADSBee 
{
   public:
    ...
    struct ADSBeeConfig 
    {
        ...
        uint16_t tl_pwm_pin = 9;      // GPIO 9 используются как выходы ШИМ для установки пороговых напряжений аналогового компаратора.
        uint16_t tl_adc_pin = 27;           // GPIO 26-27 используются как входы АЦП для считывания пороговых напряжений аналогового компаратора после фильтра ВЧ.
        uint16_t tl_adc_input = 1;
        uint16_t rssi_adc_pin = 28;         // GPIO 28 используется как вход АЦП для уровня мощности последнего декодированного пакета.
