Создам программу приема пакетов ADS-B для RP2040 согласно вашим требованиям:
...
// Очереди пакетов
std::queue<ADSBPacket> packet_queue;
std::queue<ADSBPacket> processed_queue;
...
// UART2
HardwareSerial uart2(UART_RX_PIN, UART_TX_PIN);
...
void setup() {
    ...
    attachInterrupt(digitalPinToInterrupt(INPUT_PIN_1), irq_handler_1, RISING);
    attachInterrupt(digitalPinToInterrupt(INPUT_PIN_2), irq_handler_2, RISING);
    ...
    Serial.println("Core 0: Processing packets");
    Serial.println("Core 1: Receiving packets");
}

void setup1() {
    // Инициализация для ядра 1
    Serial.println("Core 1 ready for packet reception");
}
...
UART2 вывод - структурированные данные в JSON формате
